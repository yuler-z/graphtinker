	...
	vector<edge_nt> &edge_block_array_c)
{
	...
	if (geni == 1)
	{
		if (currworkblockaddr >= edge_block_array_m.size())
		{
			cout << "bug! : addr out-of-range4 (supervs) " << endl;
		}
		if (edge_block_array_m[currworkblockaddr].clusterinfo.flag != VALID)
		{
	...
	{
		if (currworkblockaddr >= edge_block_array_c.size())
		{
			cout << "bug! : addr out-of-range5 (supervs) " << endl;
		}
		if (edge_block_array_c[currworkblockaddr].clusterinfo.flag != VALID)
		{
