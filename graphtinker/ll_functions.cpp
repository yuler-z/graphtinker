	...
	unsigned int geni)
{
	...
	if (lvaindex >= ll_lva.size())
	{
		cout << "graphtinker::ll_insert : out-of-range 1 lvaindex = " << lvaindex << ", ll_lva.size() = " << ll_lva.size() << endl;
	}
#endif
...
#ifdef EN_BUGCHECK
		if ((ll_eba_tracker->ptraddr) >= ll_edge_block_array.size())
		{
	...
	else if (entity.flag == VALID)
	{
		if ((entity.totaledgecount % LLEDGEBLOCKSIZE) == 0)
		{
			...
			if (currentlocalbaseaddr >= ll_edge_block_array.size())
			{
				cout << "graphtinker::ll_insert : out-of-range 22, entity.lastlocalbaseaddr = " << entity.lastlocalbaseaddr << ", ll_edge_block_array.size() = " << ll_edge_block_array.size() << endl;
			}
...
/// submit changes
	...
	if (lvaindex >= ll_lva.size())
	{
		cout << "bug! : out-of-range 4 (GraphTinkerLL). lvaindex = " << lvaindex << ", ll_lva.size() = " << ll_lva.size() << endl;
	}
#endif
	...
	unsigned int geni)
{
	...
	if (localaddr > 0)
	{
		...
		else
		{
			...
			// pop-out, pop-in
			if (localbaseaddr >= ll_edge_block_array.size())
			{
			...
			if (ll_localbaseaddrptr >= ll_edge_block_array.size())
			{
			...
			// pop in
			if (ll_edge_block_array[ll_localbaseaddrptr].ll_edgeblock[ll_localaddrptr].flag == VALID)
			{
	...
	else if (localaddr == 0)
	{
		...
		if (entity.lastlocalbaseaddr >= ll_edge_block_array.size())
		{
		...
		if (prevcptr != NAv)
		{
			if (prevcptr >= ll_edge_block_array.size())
			{
				cout << "bug! : addr out-of-range69. prevcptr : " << prevcptr << ", ll_edge_block_array.size() : " << ll_edge_block_array.size() << " (gds_ll) " << endl;
			}
		}
		unsigned int prevlocalbaseaddr;
		if (prevcptr != NAv)
		{
			prevlocalbaseaddr = ll_edge_block_array[prevcptr].metadata.currcptr;
		} //****
		if (prevcptr != NAv)
		{
		...
		if (((localbaseaddr == ll_localbaseaddrptr) && (localaddrofvalidedge == ll_localaddrptr)) || (prevcptr == NAv))
		{
			...
			if (prevcptr != NAv)
			{
				ll_edge_block_array[prevcptr].metadata.nextcptr = NAv;
			} //****
	...
	vector<ll_edgeblock_t> &ll_edge_block_array)
{
	...
	if (moduleparams.ll_localbaseaddrptr_x >= ll_edge_block_array.size())
	{
