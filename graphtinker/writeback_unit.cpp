...
#ifdef EN_CRUMPLEINONDELETE
	,
	unsigned int lastgenworkblockaddr, vector<supervertex_t> &svs
#ifdef EN_LLGDS
	,
...
)
{
		...
		if (writebackunitcmd.markasclustered == YES)
		{
			...
			else if ((subblockid == (subblocksperpage - 1)) && (geni > 1))
			{
				...
				{ /// last edgeblock was in generation 1
					if (lastgenworkblockaddr >= edge_block_array_m.size())
					{
						cout << "graphtinker::writeback_unit : out-of-range34" << endl;
					}
					if (edge_block_array_m[lastgenworkblockaddr].clusterinfo.flag != VALID)
					{
				...
				{
					if (lastgenworkblockaddr >= edge_block_array_c.size())
					{
						cout << "graphtinker::writeback_unit : out-of-range35" << endl;
					}
					if (edge_block_array_c[lastgenworkblockaddr].clusterinfo.flag != VALID)
					{
	...
	if (writebackunitcmd.writeback == YES)
	{
		...
		if (geni == 1)
		{
			if (writebackunitcmd.addr >= edge_block_array_m.size())
			{
		...
		{
			if (writebackunitcmd.addr >= edge_block_array_c.size())
			{
		...
		if (geni == 1)
		{
			edge_block_array_m[writebackunitcmd.addr] = *edgeblock;
		}
		else
		{
			edge_block_array_c[writebackunitcmd.addr] = *edgeblock;
		}
