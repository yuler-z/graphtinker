...
// metadata (for delete and crumple in)
	...
	void deleteandcrumplein_unit(
		...
		margin_t wblkmargin,
		margin_t subblkmargin,
		unsigned int geni, deleteandcrumpleincmd_t deleteandcrumpleincmd, vector<supervertex_t> &svs, vector<vertexid_t> &freed_edgeblock_list);
...
#ifdef EN_LLGDS
		,
		vector<ll_edgeblock_t> &ll_edge_block_array, vector<ll_logicalvertexentity_t> &ll_lva, ll_eba_tracker_t *ll_eba_tracker
#endif
	);
private:
};
