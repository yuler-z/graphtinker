...
void graphtinker::insert_edge(unsigned int src, unsigned int dst, unsigned int ew)
{
	...
	if (sgh_for_xvtxid == SELF)
	{
		src = translator_handler.get_localvid((vertexid_t)src);
	}
	if (sgh_for_xadjvtxid == SELF)
	{
		dst = translator_handler.get_localvid((vertexid_t)dst);
	}
...
void graphtinker::insert_edge(unsigned int src, unsigned int dst, unsigned int ew, vertices &external_vertices_handler)
{
	...
	if (sgh_for_xvtxid == SELF)
	{
		src = (unsigned int)translator_handler.get_localvid((vertexid_t)src);
	}
	if (sgh_for_xadjvtxid == SELF)
	{
		dst = (unsigned int)translator_handler.get_localvid((vertexid_t)dst);
	}
	check_whether_to_resize_edgeblockarray_m(src);
...
void graphtinker::insert_edge(unsigned int src, unsigned int dst, unsigned int ew, translator &translator_handler)
{
	if (src > vertex_range)
	{
		cout << "graphtinker::insert_edge : bug, out of range19. src : " << src << ", vertex_range : " << vertex_range << endl;
	}
	unsigned int edgeupdatecmd = INSERTEDGE;
	if (sgh_for_xvtxid == OTHER)
	{
		src = (unsigned int)translator_handler.get_localvid((vertexid_t)src);
	}
	if (sgh_for_xadjvtxid == OTHER)
	{
		dst = (unsigned int)translator_handler.get_localvid((vertexid_t)dst);
	}
	check_whether_to_resize_edgeblockarray_m(src);
...
void graphtinker::insert_edge(unsigned int src, unsigned int dst, unsigned int ew, vertices &external_vertices_handler, translator &translator_handler)
{
	if (src > vertex_range)
	{
		cout << "graphtinker::insert_edge : bug, out of range199. src : " << src << ", vertex_range : " << vertex_range << endl;
	}
	unsigned int edgeupdatecmd = INSERTEDGE;
	if (sgh_for_xvtxid == OTHER)
	{
...
//LL unit
	...
	if (updatev != OFF)
	{
		...
		if (sgh_for_xvtxid == SELF)
		{
			xvtxid = translator_handler.get_globalvid(edge.xvtx_id);
		}
		if (sgh_for_xadjvtxid == SELF)
		{
			xadjvtxid = translator_handler.get_globalvid(edge.xadjvtx_id);
		}
...
#ifdef EN_LLGDS
			,
			ll_edge_block_array, ll_lva, &ll_eba_tracker
#endif
		);
#endif
		//***********************************************************************************************
	...
	while (true)
	{
		for (unsigned int i = 0; i < len; i++)
		{
			...
			for (unsigned int t = 0; t < work_blocks_per_page; t++)
			{
				...
				if (_geni == 1)
				{
					edgeset = edge_block_array_m[(ebaoffset + t)];
				}
				else
				{
					edgeset = edge_block_array_c[(ebaoffset + t)];
				}
...
{
	unsigned int uniqueedgecount = 0;
	for (unsigned int i = 0; i < ll_edge_block_array.size(); i++)
	{ //ll_edgeblock_array_size
		for (unsigned int j = 0; j < LLEDGEBLOCKSIZE; j++)
		{
			if (ll_edge_block_array[i].ll_edgeblock[j].flag == VALID)
			{
				uniqueedgecount += ll_edge_block_array[i].ll_edgeblock[j].edgew;
			}
		}
	}
	return uniqueedgecount;
